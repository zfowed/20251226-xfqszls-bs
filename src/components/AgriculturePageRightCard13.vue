...
async function loadVideoStationList () {
  ...
  const defaultVideoItem = playList.value.find(item => item.stcd === '1000008') || playList.value[0]
  selectedVideoId.value = defaultVideoItem.id
  await updateCurrentVideoUrl(defaultVideoItem)
}
